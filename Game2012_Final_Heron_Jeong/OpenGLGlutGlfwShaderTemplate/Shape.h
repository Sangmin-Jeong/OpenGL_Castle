...
public:
	...
	{
		// Resize shape_normals so we can use [].
		shape_normals.resize(verticeCount);
		// Calculate the normals of each triangle first.
		for (unsigned i = 0; i < indiceCount; i += 3)
		{
			unsigned in0 = indices[i] * 3;		// Starting index of first vertex in shape_vertices.
			unsigned in1 = indices[i + 1] * 3;	// Starting index of second vertex in shape_vertices.
			unsigned in2 = indices[i + 2] * 3;	// Starting index of third vertex in shape_vertices.
